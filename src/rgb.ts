...
export class RGB {
  ...
  correctLuma(desired: number): RGB {
    const m = desired - this.luma();
    if (Math.abs(m) >= 0.5) console.log("@@@ change", m)
    // return this;
  ...
  toHCY(): number {
    ...
    const hue = max == this.red ?
      (this.green - this.blue) / chroma :
      (max == this.green ? this.blue - this.red + 2 : this.red - this.green + 4);
    const luma = 0.2126 * this.red + 0.7152 * this.green + 0.0722 * this.blue;
  ...
  blendY(other: RGB, alpha: number): RGB {
    const nalpha = 1 - alpha;
    const luma = this.luma() * alpha + other.luma() * nalpha;
    return new RGB(
...
export function quantize4to2(colors: RGB[]): RGB[] {
  ...
  if (merge[0][0] == merge[1][0] || merge[0][1] == merge[1][0]) {
    ...
    // two pairs
    const c1 = colors[merge[0][0]].blendx(colors[merge[0][1]], 0.5);
    const c2 = colors[merge[1][0]].blendx(colors[merge[1][1]], 0.5);
  ...
  rv[triplet[0]] = rv[triplet[1]] = rv[triplet[2]] = colors[triplet[0]].mix3x(colors[triplet[1]], colors[triplet[2]]);
  return rv;
}
